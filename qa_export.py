...
def get_unpalleted_devices(start_date: date = None, end_date: date = None) -> List[Dict[str, object]]:
    try:
        from manager.bottleneck import get_unpalleted_devices as _fn
    except Exception:
        from .manager.bottleneck import get_unpalleted_devices as _fn
...
def get_unpalleted_devices_recent(days_threshold: int = 7) -> List[Dict[str, object]]:
    try:
        from manager.bottleneck import get_unpalleted_devices_recent as _fn
    except Exception:
        from .manager.bottleneck import get_unpalleted_devices_recent as _fn
    return _fn(days_threshold)
...
def get_awaiting_qa_counts_for_date(target_date: date) -> Dict[str, int]:
    ...
    try:
        from manager.bottleneck import get_awaiting_qa_counts_for_date as _fn
    except Exception:
        from .manager.bottleneck import get_awaiting_qa_counts_for_date as _fn
    return _fn(target_date)


def get_unpalleted_summary(destination: str = None, days_threshold: int = 7) -> Dict[str, object]:
    try:
        from manager.bottleneck import get_unpalleted_summary as _fn
    except Exception:
        from .manager.bottleneck import get_unpalleted_summary as _fn
    return _fn(destination, days_threshold)


def normalize_roller_name(roller_location: str) -> str:
    try:
        from manager.bottleneck import normalize_roller_name as _fn
    except Exception:
        from .manager.bottleneck import normalize_roller_name as _fn
    return _fn(roller_location)
...
def is_data_bearing_device(description: str) -> bool:
    try:
        from manager.bottleneck import is_data_bearing_device as _fn
    except Exception:
        from .manager.bottleneck import is_data_bearing_device as _fn
...
def get_roller_queue_status(days_threshold: int = 1, target_date: date | None = None, roller_whitelist: List[str] | None = None, qa_user_filter: List[str] | None = None) -> Dict[str, object]:
    ...
    """
    try:
        from manager.bottleneck import get_roller_queue_status as _fn
    except Exception:
        from .manager.bottleneck import get_roller_queue_status as _fn
    return _fn(days_threshold, target_date, roller_whitelist, qa_user_filter)


def get_stale_devices(days_threshold: int = 7) -> List[Dict[str, object]]:
    try:
        from manager.bottleneck import get_stale_devices as _fn
    except Exception:
        from .manager.bottleneck import get_stale_devices as _fn
    return _fn(days_threshold)


    # Note: Unpalleted / Stale device export sheets removed to keep exports focused.
    # The underlying `manager.bottleneck` helpers remain available if needed later.
